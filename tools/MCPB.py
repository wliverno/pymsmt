    ...
    #xstru
        ...
        if len(line) == 2:
            ...
            raise pymsmtError('More than one xstru parameter provided, '
                              'need one.')
    #incC
    elif line[0].lower() == 'include_carbon':
        if len(line) == 2:
            try:
                d = [False, True]
                if int(line[1]) not in [0, 1]:
                    raise pymsmtError('include_carbon varible needs to be 0 or 1, '
                                      '0 means no carbon bonds, 1 means including carbon bonds.')
                incC = d[int(line[1])]
            except:
                raise pymsmtError('include_carbon value is not an integer value.')
        elif len(line) == 1:
            warnings.warn('No include_carbon parameter provided. '
                          'Default value 0 is used.', pymsmtWarning)
        else:
            raise pymsmtError('More than one incC parameter provided, '
                              'need one.')
...
print('The variable include_carbon is : ', incC)
#==============================================================================
# Related define
#==============================================================================
