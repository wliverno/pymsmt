...
def get_matrix_from_fchk(fname, msize):
    ...
    #What if actual matrix is larger than msize
    msize2 = int(0.5*(numpy.sqrt(8*elenums2+1) - 1))
    fcmatrix = numpy.array([[float(0) for x in range(msize2)] for x in range(msize2)])
    ...
    # Use submatrix if msize2>msize
    fcmatrix = fcmatrix[numpy.ix_(range(msize), range(msize))]
    return fcmatrix
