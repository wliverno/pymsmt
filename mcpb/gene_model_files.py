...
def build_standard_model(mol, reslist, cutoff, msresids, outf, ionids,
    ...
    # What about ion-ion bonding?? Added:
    for i in range(len(ionids)-1):
        ion1 = ionids[i]
        for ion2 in ionids[i+1:]:
            dis = calc_bond(mol.atoms[ion1].crd, mol.atoms[ion2].crd)
            print(ion1, ion2, dis)
            if (dis <= cutoff):
                print("LINK", str(ion1)+'-'+mol.atoms[ion1].atname,
                    str(ion2)+'-'+mol.atoms[ion2].atname, file=stpff)
